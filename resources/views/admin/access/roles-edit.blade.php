...
<div class="max-w-7xl mx-auto px-4 py-12" x-data="{ 
    ...
    <form action="{{ route('admin.access.roles.update', $role->id) }}" method="POST">
        ...
        <div class="mb-10 bg-rose-50 border-2 border-rose-100 rounded-[2.5rem] p-8 flex items-start gap-6 animate-pulse">
            <div class="w-14 h-14 bg-rose-500 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-rose-500/20 shrink-0">
                <i class="ph-fill ph-warning-octagon text-3xl"></i>
            </div>
            <div>
                <h3 class="text-lg font-black text-rose-900 leading-tight mb-2">Advertencia de Segregación</h3>
                <ul class="space-y-1">
                    @foreach($conflicts as $conflict)
                    <li class="text-sm font-bold text-rose-700 flex items-center gap-2">
        ...
        <div class="mb-12 flex flex-col lg:flex-row lg:items-end justify-between gap-8 sticky top-0 z-50 bg-slate-50/90 backdrop-blur-xl py-6 -mt-6">
            <div class="flex-1">
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        <h1 class="text-3xl font-black text-slate-900 tracking-tighter">Perfil: <span class="text-amber-600">{{ $role->name }}</span></h1>
                        <p class="text-sm font-medium text-slate-400 italic">Simplificación de capacidades mediante acciones directas</p>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-4 items-center">
                    <div class="w-full md:w-80">
                        <input type="text" name="name" value="{{ old('name', $role->name) }}" required
                               ...
                               placeholder="Ej. Supervisor de Servicio">
                    </div>
