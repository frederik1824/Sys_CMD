...
<div class="max-w-4xl mx-auto px-4 py-12">
    <div class="mb-10 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-slate-900 tracking-tight">Editar Perfil</h1>
            <p class="text-slate-500 font-medium">Modifica los datos globales de {{ $user->name }}</p>
        </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden">
        <form action="{{ route('admin.access.users.update', $user) }}" method="POST" class="p-10 md:p-16">
            @csrf
            @method('PUT')
                    ...
                    <div class="relative">
                        ...
                        <select name="departamento_id" class="w-full pl-14 pr-6 py-4 bg-slate-50 border-transparent rounded-2xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500/20 transition-all appearance-none">
                            ...
                            @foreach($departamentos as $dep)
                                <option value="{{ $dep->id }}" {{ old('departamento_id', $user->departamento_id) == $dep->id ? 'selected' : '' }}>{{ $dep->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="relative">
                        ...
                        <select name="responsable_id" class="w-full pl-14 pr-6 py-4 bg-slate-50 border-transparent rounded-2xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500/20 transition-all appearance-none">
                            ...
                            @foreach($responsables as $resp)
                                <option value="{{ $resp->id }}" {{ old('responsable_id', $user->responsable_id) == $resp->id ? 'selected' : '' }}>{{ $resp->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="bg-blue-50/50 rounded-3xl p-8 mb-10 border border-blue-100/50">
                <div class="flex items-start gap-4">
                    <div class="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center flex-shrink-0">
                        <i class="ph ph-info text-blue-600 text-xl"></i>
                    </div>
                    <div>
                        <p class="text-sm font-black text-blue-900 mb-1 tracking-tight">Gestión de Seguridad</p>
                        <p class="text-xs text-blue-700/70 font-medium leading-relaxed">
                            Para cambiar la contraseña de este usuario, utiliza la función de <strong>Reset</strong> en la lista principal de nómina. Esto enviará una notificación de seguridad al registro de auditoría.
                        </p>
                    </div>
                </div>
            </div>
