...
<div class="max-w-3xl mx-auto flex flex-col gap-6">
    <div class="flex items-center gap-4">
        <a href="{{ route('sistema.estados.index') }}" class="text-slate-400 hover:text-primary transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-8">
        <form action="{{ isset($estado) ? route('sistema.estados.update', $estado) : route('sistema.estados.store') }}" method="POST" class="space-y-6">
            @csrf
            @if(isset($estado))
                @method('PUT')
            @endif
            ...
            <div class="pt-6 border-t border-slate-100 flex justify-end gap-3">
                <a href="{{ route('sistema.estados.index') }}" class="px-6 py-2.5 rounded-lg font-semibold text-slate-600 hover:bg-slate-50 transition-colors border border-transparent">Cancelar</a>
